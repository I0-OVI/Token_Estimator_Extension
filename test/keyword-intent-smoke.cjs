...
(async () => {
  ...
  const { est, extraNotes } = await runEstimateWithKeywords("hello", DEFAULT_PREDICTION_OPTIONS, "hint_only");
  assert.ok(est.totalTokensExpected > 0);
  assert.ok(extraNotes.length >= 1);

  const { est: est2 } = await runEstimateWithKeywords("refactor all", DEFAULT_PREDICTION_OPTIONS, "override");
